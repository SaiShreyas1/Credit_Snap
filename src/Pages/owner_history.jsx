...
import { 
  Menu, Home, ClipboardList, Wallet, BarChart2, 
  History, HelpCircle, Bell, UserCircle, Search, ChevronDown 
} from 'lucide-react';

// Using your imported logo
import ownerLogo from '../assets/CreditSnap_logo_Canteen.png';

export default function OwnerHistory() {
  const [searchTerm, setSearchTerm] = useState('');
  
  // Mock data matching your exact wireframe
  ...
  return (
    <div className="flex h-screen bg-gray-50 font-sans">
      ...
      {/* ================= SIDEBAR (Matches friend's code exactly) ================= */}
      <aside className="w-48 bg-[#1e293b] text-white flex flex-col justify-between">
        <div>
          <div className="p-4">
            <Menu className="w-8 h-8 cursor-pointer hover:text-[#FF9A00] transition" />
          </div>
          ...
          <nav className="mt-4 flex flex-col gap-2">
            ...
            <div className="py-3 px-4 flex flex-col items-center justify-center text-gray-300 hover:text-white cursor-pointer opacity-70 transition">
              ...
              <span className="text-sm text-center leading-tight">Edit Menu</span>
            </div>

            <div className="py-3 px-4 flex flex-col items-center justify-center text-gray-300 hover:text-white cursor-pointer opacity-70 transition">
              <Wallet className="w-6 h-6 mb-1" />
              <span className="text-sm text-center leading-tight">Active Debts</span>
            </div>

            <div className="py-3 px-4 flex flex-col items-center justify-center text-gray-300 hover:text-white cursor-pointer opacity-70 transition">
              <BarChart2 className="w-6 h-6 mb-1" />
              <span className="text-sm text-center leading-tight">Analytics</span>
            </div>

            {/* ACTIVE HISTORY TAB - Using your exact #FF9A00 color code */}
            <div className="bg-[#FF9A00] mx-2 py-3 px-4 rounded-xl flex flex-col items-center justify-center cursor-pointer shadow-lg">
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        ...
        {/* Top Navbar (Matches friend's code exactly) */}
        <header className="h-16 bg-[#f4f7fb] border-b flex justify-between items-center px-4 shadow-sm z-10 overflow-hidden">
          ...
          <div className="flex items-center gap-4 pr-2">
            <Bell className="w-6 h-6 text-gray-700 cursor-pointer hover:text-[#FF9A00] transition" />
            <UserCircle className="w-8 h-8 text-gray-900 cursor-pointer hover:text-[#FF9A00] transition" />
          </div>
        </header>
        ...
        <main className="p-8 overflow-y-auto flex-1">
          ...
          <div className="flex flex-col md:flex-row justify-between items-center mb-8 gap-4">
            {/* Search Bar with your grey #D9D9D9 border */}
            ...
            {/* Filter and Sort buttons with your #FF9A00 color */}
            <div className="flex gap-4">
              <button className="bg-[#FF9A00] text-white px-5 py-2.5 rounded-xl text-sm font-semibold flex items-center gap-2 shadow-sm hover:opacity-90 transition">
          ...
          <div className="space-y-4">
            {filteredData.length === 0 ? (
              ...
              filteredData.map((record, index) => (
                <div key={index} className="bg-white p-5 rounded-xl shadow-sm border border-[#D9D9D9] flex justify-between items-center">
                  <div>
